...
public class MazeCatController : MonoBehaviour
{
  [SerializeField]
  ...
  public void moveDir(){
    if (mcurrentState == MoveState.Moving)
      return;
    //要先知道有哪些方向可以移動
    Dir[] dir_arr = MazeManager._MazeManager.GetMaze().DonthasWall(currentx, currenty);

    Dir dir = dir_arr[UtilityHelper.Random(0, dir_arr.Length)]; 
  ...
  void move(){
    ...
    if (mcurrentState == MoveState.Stop)
      return;
    ...
    if ((TargetCell.position - currentposi).magnitude <= dis){
      ...
      MazeManager._MazeManager.ArrivalCell("cat", TargetCell);
  ...
  public bool IsStop(){
    return mcurrentState == MoveState.Stop;
  }
